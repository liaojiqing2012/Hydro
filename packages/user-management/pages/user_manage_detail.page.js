...
const page = new NamedPage('user_management_detail', () => {
  ...
  $('#edit-user-form').on('submit', async function(e) {
    ...
    try {
      ...
      if (response.success) {
        ...
        $('#display-email').text(formData.mail);
  ...
  $('#reset-password').on('click', async function() {
    ...
    try {
      const response = await request.post(`/manage/users/${uid}`, {
        ...
        password: prompt(i18n('Enter new password for user {0}:', username))
      });
      
      if (response.success) {
        Notification.success(i18n('Password reset successfully'));
      } else {
